...
namespace ECommerceApp.UnitTests.Sales.Orders
{
    public class SnapshotOrderItemsJobTests
    {
        ...
        public async Task ExecuteAsync_NoItems_ShouldReportSuccessNoOp()
        {
            ...
            _productResolver.Verify(r => r.ResolveAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()), Times.Never);
        ...
        public async Task ExecuteAsync_ItemsWithResolvedSnapshots_ShouldCallSetSnapshotsAndReportSuccess()
        {
            ...
            var snapshot1 = new OrderProductSnapshot("Product A", "a.jpg");
            var snapshot2 = new OrderProductSnapshot("Product B", null);

            _orderItemRepo
                .Setup(r => r.GetUnsnapshottedOrderItemsAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(new List<OrderItem> { item1, item2 });
            _productResolver
                .Setup(r => r.ResolveAsync(10, It.IsAny<CancellationToken>()))
                .ReturnsAsync(snapshot1);
            _productResolver
                .Setup(r => r.ResolveAsync(20, It.IsAny<CancellationToken>()))
        ...
        public async Task ExecuteAsync_ProductNotFound_ShouldSkipItemAndStillSetOthers()
        {
            ...
            _productResolver
                ...
                .ReturnsAsync(snapshot1);
            _productResolver
                .Setup(r => r.ResolveAsync(99, It.IsAny<CancellationToken>()))
                .ReturnsAsync((OrderProductSnapshot?)null);
