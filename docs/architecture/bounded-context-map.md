...
| **AccountProfile** (`UserProfile`)  | Behavioral aggregate      | Rich domain model, owned `Address`, own `UserProfileDbContext`; guest→account linking uses the `IVerificationCodeClient` ACL into `Supporting/Verification` — see ADR-0005 and ADR-0030 §6                                                                                                                                          | ✅ **Switch live** — `ProfileController` in `Areas/AccountProfile`; legacy `CustomerController`/`AddressController`/`ContactDetailController` deleted |
...
| **Inventory/Availability**          | Behavioral aggregate      | Counter aggregate (`StockItem`), `Reservation`, `ProductSnapshot` ACL, `AvailabilityDbContext`, `RowVersion` optimistic locking, deferred `StockAdjustmentJob` with command coalescing, publishes `AvailabilityChanged` outbound — see ADR-0011. Note: `SoftReservation` moved to Presale/Checkout (ADR-0012). Stock-hold release on payment timeout is event-driven only (`PaymentExpiredHandler` reacting to Payments' `PaymentExpired`) — the redundant standalone `PaymentWindowTimeoutJob` deferred timer was retired 2026-08-17 (payments-atomic-switch.md Step 6). | ✅ **Switch live** — `StockController` in `Areas/Inventory`; legacy controller + views deleted                                                        |
...
| **Storefront §3 — category strip** (`ByCategory` query + view + UI strip)                               | [ADR-0012](../adr/0012-presale-checkout-bc-design.md)                        | ✅ Done — `ICatalogNavigationService` injected in `StorefrontController`; category badge strip in `Index.cshtml`                                             |
...
| **`IOrderExistenceChecker` contract deduplication** — defined in both Coupons and Fulfillment Contracts | —                                                                            | ✅ Done — interface fully deleted; `Application/Sales/Shared/` folder removed; replaced by `IModuleClient.SendAsync(new OrderExistsQuery(...))` (2026-05-10) |
