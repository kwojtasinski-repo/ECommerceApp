# RAG Architecture — v2 Multi-Project Design (HISTORICAL, NOT IMPLEMENTED)

> **Status: HISTORICAL DESIGN PROPOSAL — NOT IMPLEMENTED.**
> This document describes a v2 architecture that was proposed in May 2026 and
> ultimately superseded by the dual-stack design now in production. The current
> system uses two parallel implementations (Python in `tools/rag/`, .NET in
> `tools/rag-dotnet/`) with an HTTP/stdio MCP server per stack, remote ingest
> via `POST /ingest/{collection}`, and an ApiKey middleware layer — **none of
> the manifest/`.copilot-rag.yaml`/single-shared-server pieces below were built**.
>
> **For the current architecture, see [`rag-architecture.md`](rag-architecture.md).**
> **For the deployed remote-ingest design, see [ADR-0028](../adr/0028/0028-remote-multitenant-rag-ingest.md)
> and its [.NET](../adr/0028/tech-details-dotnet.md) / [Python](../adr/0028/tech-details-python.md) tech-details files.**
>
> The document is retained because some of its ideas (incremental ingest with
> hash manifest, `/rag-sync` agent, eval pipeline, weight system) inform the
...
> Status (May 2026): **Design updated — incremental ingest, MCP startup auto-check, `/rag-sync` agent, eval pipeline added.**
