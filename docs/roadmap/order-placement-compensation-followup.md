...
> **Status:** All four workstreams are done — 1 (cart restore), 2 (Outbox pattern, all 5 phases), 3 (F4
> cleanup, done 2026-08-17), and 4 (docs correction). **Next up:** resume the generic saga engine work in
> [`generic-saga-orchestration-proposal.md`](./generic-saga-orchestration-proposal.md) (Phase 1, engine
> core) — see table below.
...
| **3** | **F4 cleanup** | Deduplicate `ShipmentDelivered/Failed/PartiallyDelivered` handlers into shared logic — preparatory, not risk-driven (new requirements are expected to land here later) | **Done (2026-08-17), validated PASS.** Extracted a shared `internal abstract class ShipmentStockOutcomeHandlerBase` (`ECommerceApp.Application/Inventory/Availability/Handlers/ShipmentStockOutcomeHandlerBase.cs`) owning the transaction/guard/failure-collection/`StockReconciliationRequired`-emit/commit flow — an inheritance-based template method, deliberately **not** a static helper/utility class (explicit user preference: helpers fragment logic and cause future cross-cutting refactors). Each of the three handlers now only builds its `IReadOnlyList<StockOperation>` from its message shape and calls `ProcessAsync`. As part of the dedup, `ShipmentFailedHandler` was also unified onto the same guarded shape as the other two (promoted from plain `IMessageHandler<ShipmentFailed>` to `IIdAwareMessageHandler<ShipmentFailed>`, gained an `IProcessedMessageGuard` dependency, transaction now opens unconditionally instead of only when failures occur) — a deliberate behavior change for consistency, not just a refactor; confirmed safe because `ShipmentFailed` is published exclusively through `IOutboxWriter` (`ShipmentService.MarkAsFailedAsync`), so `outboxMessageId` is always present at dispatch (`OutboxDispatcher` → `ModuleClient.PublishAsync(imessage, message.Id)`) and `ModuleClient` detects `IIdAwareMessageHandler` via runtime `IsInstanceOfType`, independent of the DI registration type — no DI/registration changes were needed. Verified: `dotnet build` 0 errors; `ECommerceApp.UnitTests` 1142/1142 (added an "already processed → skip" guard test to all 3 handler test classes plus a constructor/behavior update for `ShipmentFailedHandlerTests`); `ECommerceApp.IntegrationTests` 250/250, including `SalesOrdersDuplicateDeliveryTests` (`ShipmentFailed` redelivery through the real dispatch chain) and `InventoryEventChainTests` (`ShipmentFailed_WithConfirmedStock_ShouldTriggerReconciliationAlert`), both unaffected by the transaction-shape change. |
