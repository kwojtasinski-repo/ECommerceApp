...
| **Chunked image upload**                           | [`chunked-upload.md`](./chunked-upload.md)                 | —                                                                            | ✅ V1 complete — `IChunkedUploadService` + `UploadSessionStore` live; `InitUpload`/`UploadChunk` in Web + API `ImageController`; chunk progress UI in `AddItemNew`/`EditItemNew` |
...
| **F3** | **Saga / Orchestrator pattern evaluation**              | Some multi-step flows (e.g., PlaceOrder → Payment → Inventory → Fulfillment) are currently choreography-based (event chains). Two structural gaps identified: (1) no compensation on partial failure — if Inventory.ReserveStock fails after Payments.CreatePayment succeeds, there is no rollback signal; (2) no saga state persistence — failed event chains leave no structured breadcrumb. **Option A** (recommended first): add `OrderPlacementFailed` compensation handlers on top of the existing `IMessageBroker` infrastructure. **Option B** (future): `OrderLifecycleSaga` process manager entity with persisted step state — requires Outbox pattern first. Do not refactor prematurely. | When we confirm a real compensation gap in production, OR when Option A is agreed to be insufficient                                                            | [`saga-pattern.md`](./saga-pattern.md) — full analysis, gap map, sequencing |
