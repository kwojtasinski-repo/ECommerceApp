...
| **F5** | **ModuleClient evolution — universal cross-BC adapter** | `ModuleClient.PublishAsync` already uses `GetServices()` + `foreach` — KI-007 ✅ fixed. Beyond the bug fix, the `ModuleClient` concept (generic cross-module dispatch via `IServiceProvider`) is architecturally superior to the current per-BC adapter classes (`OrderExistenceCheckerAdapter`, `CatalogClientAdapter`, `StockAvailabilityCheckerAdapter`, etc.). Phase 2: evolve `ModuleClient` into a universal cross-BC communication mechanism that supports query/request-response semantics — this would eliminate the need to write a new adapter class every time one BC needs data from another.                                                                                           | Phase 2 (query support): when adapter usage patterns stabilise and a concrete need is identified.                                                               | —                                                                           |
