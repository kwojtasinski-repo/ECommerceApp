...
- [ ] `ExceptionResponse` has `IReadOnlyList<ErrorCodeDto>? Codes` property
- [ ] `ErrorMapToResponse` maps `BusinessException._codes` into `ExceptionResponse.Codes`
- [ ] Non-`BusinessException` mappings pass `codes: null` — no regression for `500` responses
- [ ] `ExceptionResponse.ToString()` serializes `codes` with camelCase; omits key when null
- [ ] `errors.js:showErrorFromResponse` handles both `data.codes` (array) and `data.response` (flat fallback)
- [ ] `showError(errorsArray)` is unchanged — still iterates `[{code, parameters}]` array
