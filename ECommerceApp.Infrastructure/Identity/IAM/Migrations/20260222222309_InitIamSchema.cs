...
{
    /// <summary>
    /// Creates all ASP.NET Core Identity tables inside the [iam] schema with clean names
    /// (Users, Roles, UserRoles, UserClaims, UserLogins, UserTokens, RoleClaims)
    /// managed by IamDbContext — separate from the legacy [dbo].[AspNetXxx] tables in Context.
    ///
    /// Intent: enforce the IAM bounded-context boundary at the database level (ADR-0004).
    /// This is a CREATE-only migration — no existing data is touched.
    /// The [dbo].[AspNetXxx] tables remain in place and continue to serve the current
    /// Identity registration until the switch migration runs.
    ///
    /// Rollback: apply Down() which drops the [iam] schema tables entirely.
    /// </summary>
    public partial class InitIamSchema : Migration
    {
        /// <inheritdoc />
