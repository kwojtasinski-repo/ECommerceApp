...
namespace ECommerceApp.Application.Sales.Coupons.Handlers
{
    internal sealed class TagNameChangedHandler : IMessageHandler<TagNameChanged>
    {
        ...
        public TagNameChangedHandler(IScopeTargetRepository scopeTargets)
        {
            _scopeTargets = scopeTargets;
        }

        public async Task HandleAsync(TagNameChanged message, CancellationToken ct = default)
        {
            var targets = await _scopeTargets.GetByScopeTypeAndTargetIdAsync(
                CouponRuleNames.PerTag, message.TagId, ct);
