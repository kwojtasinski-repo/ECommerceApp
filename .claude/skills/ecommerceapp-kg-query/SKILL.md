...
description: Answer questions about ECommerceApp's structure (modules, entities, endpoints, pages, message/query contracts, jobs, RBAC) by querying the code-derived knowledge graph via the `ecommerceapp-kg` MCP server. Use whenever the user asks about blast radius, module ownership/dependencies, action exposure, job scheduling, orphaned contracts, or RBAC coverage — instead of grepping the codebase.
---

# ECommerceApp Knowledge Graph Query Skill

Answer questions about ECommerceApp's real structure by querying the knowledge
graph instead of reading source. The KG models Modules (14 bounded contexts),
Entities, Repositories, Actions, Endpoints, Pages, ScriptModules, the async
Message/MessageHandler and sync Query/QueryHandler contracts, Jobs, and RBAC
(Role/Policy). Source of truth: `tools/kg/seed/ontology.cypher` (schema) + the
generated seed from `kg-codegen`. Design rationale: ADR-0031 and
`docs/architecture/knowledge-graph-ontology-design.md`.

## When to use this skill

Use when the question can be answered from structural facts about the codebase:
...
Do **not** use the KG for: effort/time estimates, "which architectural
pattern/archetype", test coverage, git authorship/history, or runtime
behavior (call frequency, actual message volume) — these are explicitly out
of scope, see `docs/reference/kg-mcp-tools.md`.
...
1. **Load the MCP tool schemas first** (they're deferred):
   ```
   ToolSearch(query: "select:mcp__ecommerceapp-kg__GetNodeNeighbors,mcp__ecommerceapp-kg__GetBlastRadius,mcp__ecommerceapp-kg__GetNodeDependencies,mcp__ecommerceapp-kg__GetModuleDependencies,mcp__ecommerceapp-kg__GetModuleOwnership,mcp__ecommerceapp-kg__GetActionExposure,mcp__ecommerceapp-kg__GetOrphanContracts,mcp__ecommerceapp-kg__GetJobSchedulers,mcp__ecommerceapp-kg__GetGovernedActions,mcp__ecommerceapp-kg__FindStructurallySimilarActions", max_results: 10)
   ```
   If the exact names return nothing, search by keyword instead — the
   MCP-qualified prefix can vary by client.
...
3. **`nodeId` is exact and case-sensitive** — usually a fully-qualified type
   ...
   or a module name (e.g. `Payments`). See `references/labels.md` for valid
...
- `references/labels.md` — node labels, the 14 module names, and the
  relationship vocabulary (mirrors `tools/kg/seed/ontology.cypher`).
- `docs/reference/kg-mcp-tools.md` — authoritative tool contracts, error
  semantics, explicit out-of-scope questions.
- `docs/architecture/knowledge-graph-ontology-design.md` — ontology design
  rationale (layers, guardrails, target questions).
- ADR-0031 — the decision record.
