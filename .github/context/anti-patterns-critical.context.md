...
- **No mixing unrelated MCP families for the same atomic intent** — pick the relevant source per [mcp-routing.instructions.md](../instructions/mcp-routing.instructions.md).
...
- **No raw `fetch_webpage` for project-related URLs** (ADRs, docs, package registries, GitHub links cited in repo work) — use the approved project URL retrieval path.
...
- **No computing hashes, math, regex transformations, or repo-wide derivations from training-data memory** — use bounded local tools and report the command or evidence used.
