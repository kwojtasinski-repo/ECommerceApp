...
*Last updated: 2026-06-05 (Coupons stacking strategy: Rule A — fixed-value coupon rejected when value > original total; Rule B — effective-price fail-fast (≤ 0 → `NoDiscountProduced`), reduction capped at remaining balance; `OrderPriceAdjusted.NewPrice` corrected to `effectivePrice − actualReduction` for multi-coupon accuracy. 44 coupon unit tests. 1019/1020 total tests passing (1 pre-existing Catalog arch failure).)*
...
| **Sales/Coupons Slice 1** | ✅ **Switch live** — `CouponController` migrated, legacy UI controllers removed, nav updated. Legacy service DI retained for Step 8 (OrderService dependency). **Slice 2 gaps A–F complete**: rule pipeline wired into `ApplyCouponAsync`, multi-coupon guard, audit trail (`CouponApplicationRecord`), `OrderPriceAdjusted` publication, `NoDiscountProduced` guard (no-rule coupons rejected), handler rewrite (multi-coupon cancellation + audit reversal), admin UI switched to `CreateCouponAsync`. **Stacking strategy implemented**: Rule A — fixed-value coupon rejected when `discountValue > originalTotal`; Rule B — fail-fast when `effectivePrice ≤ 0`, cap reduction to remaining balance when coupon exceeds it; `OrderPriceAdjusted.NewPrice` corrected to `effectivePrice − actualReduction`. 44 unit tests passing. DB migration pending approval. | [ADR-0016](../docs/adr/0016-sales-coupons-bc-design.md) |
