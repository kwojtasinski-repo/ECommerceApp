...
| **Sales/Payments BC** | Domain ✅ Application ✅ Infrastructure ✅ Unit tests ✅ Integration tests ✅ DB migrations ✅ approved — **Web Area controller + views ✅ done**; legacy `PaymentHandler` retained (not modified — will simply stop being called after switch) | **Unblocked** — Orders switch is now live; Payments atomic switch can proceed |
| **Presale/Checkout BC** | Checkout Area controller ✅ + views ✅ (Cart enriched with product names via `ICatalogClient`). PlaceOrder profile prefill ✅ done — `customerId` resolved from `UserProfileDetailsVm.Id`, all fields prefilled from first address | **Unblocked** — Orders switch is now live; Slice 2 can proceed |
...
| Area | Summary | ADR |
|---|---|---|
| **Sales/Orders — switch live** | All acceptance criteria met. Web Area controllers + views, API tiered access (G1–G8: TrustedApiUser policy, api:purchase claim, ownership checks, MaxApiQuantityFilter, AddToCartDtoValidator, WebOptions + paymentUrl, legacy OrderController/OrderItemController swapped to new async services with write endpoints returning 410 Gone). Nav links updated. 1457/1457 tests passing. Legacy code retained for Step 8 cleanup. | [ADR-0014](../docs/adr/0014-sales-orders-bc-design.md), [ADR-0024](../docs/adr/0024-controller-routing-strategy.md), [ADR-0025](../docs/adr/0025-api-tiered-access-trusted-purchase-policy.md) |
