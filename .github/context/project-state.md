...
*Last updated: 2026-06-05 (Item 8 complete: Coupons Slice 2 atomic switch done — dead `AddCouponAsync` removed, `IDbContextMigrator<CouponsDbContext>` wired, no legacy code remains. Item 6 complete: `RefreshTokenCleanupTask` + `IRefreshTokenRepository.DeleteExpiredAsync` + 4 unit tests. 1023/1024 total tests passing.)*
