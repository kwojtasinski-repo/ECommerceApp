...
/// <summary>
/// Query-time multilingual expansion preprocessor — <b>DB-only</b> variant.
///
...
///   <item>If empty, performs <b>no expansion</b> — the mounted YAML is never consulted.
///         This isolates tenants from operator-side defaults in true multitenant SaaS
///         deployments.</item>
/// </list>
///
/// Ingest path is a no-op (document vectors stay pure representations of source text).
///
/// For single-org / STDIO deployments where mounted fallback is the desired behavior,
/// register <see cref="MountedFallbackGlossaryExpansionPreprocessor"/> instead.
/// Selection is via the <c>RAG_GLOSSARY_FALLBACK</c> env var in <c>Program.cs</c>.
/// </summary>
public sealed class DbOnlyGlossaryExpansionPreprocessor(
    ...
    RagSession session) : IEmbedderPreprocessor
{
    public async Task<string> ProcessAsync(string text, EmbedContext ctx, CancellationToken ct = default)
    {
        ...
        if (payload.GlossaryEntries.Count == 0) return text;

        var glossary = MultilingualGlossary.FromEntries(payload.GlossaryEntries);
